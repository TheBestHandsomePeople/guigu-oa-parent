...
public class SysMenuServiceImpl extends ServiceImpl<SysMenuMapper, SysMenu> implements SysMenuService {
  ...
  */
    ...
    public void removeMenuById(Long id) {
        ...
        if (count > 0) {
            throw new JinCaiException(201,"菜单不能删除");
        }
    ...
    public List<SysMenu> findMenuByRoleId(Long roleId) {
        ...
        // 3 根据获取的菜单id，获取对应菜单对象
        List<Long> menuIdList = sysRoleMenuList.stream().map(c -> c.getMenuId()).collect(Collectors.toList());
            // 3.1 拿着菜单id 和所有菜单集合里面的id进行比较。如果相同则封装
    ...
    public List<RouterVo> findUserMenuListByUserId(Long userId) {
        ...
        if (userId.longValue() == 1) {
            ...
            // 1.2 若不是管理员，根据userId查询可以操作菜单列表i
        ...
        //构建成框架要求的路由结构
      List<RouterVo> routerList=  this.biuldRouter(sysMenuTreeList);

        return routerList;
    }
//    构建成框架要求的路由结构
    private List<RouterVo> biuldRouter(List<SysMenu> menus) {
...
//                    递归
                    router.setChildren(biuldRouter(children));
                }
            }
            routers.add(router);
    ...
    public List<String> findUserPermsByUserId(Long userId) {
        ...
        if (userId == 1) {
            ...
            // 1.2 若不是管理员，根据userId查询可以操作按钮列表i
            //多表关联查询：用户角色关系表，角色菜单关系表，菜单表
            sysMenuList = baseMapper.findMenuListByUserId(userId);
        }
        // 2 从查询出的数据里面，获取可以操作按钮值的list集合，返回
